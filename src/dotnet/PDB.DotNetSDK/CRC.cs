...
namespace PDB.DotNetSDK
{
  ...
  internal class CRC
  {
    private static UInt64[] crc64_tab = new UInt64[] {
    ...
    public static UInt64 Crc64(byte[] buf)
    {
      return Crc64(buf, 0, buf.Count());
    }

    public static UInt64 Crc64(byte[] buf, int offset, int len)
    {
      UInt64 crc = 0;

      for (int i = offset; i < buf.Length && i < (offset + len); i++)
      {
        crc = crc64_tab[(byte)crc ^ buf[i]] ^ (crc >> 8);
      }

      return crc;
    }

    public static UInt32 Crc32(byte[] buf, int offset, int len)
    {
      UInt64 crc64code = Crc64(buf, offset, len);
      return (UInt32)(crc64code & 0xFFFFFFFF);
    }
  }
}
