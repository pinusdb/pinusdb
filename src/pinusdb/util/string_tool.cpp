...
//包含大小写字母，数字，下划线,大写转小写
static const uint8_t allow_name_char[256] = {
...
bool StringTool::ValidUserName(const char* pUserName, size_t nameLen)
{
  if (nameLen >= PDB_USER_NAME_LEN)
    return false;

  return ValidName(pUserName, nameLen);
}

bool StringTool::ValidTableName(const char* pTabName, size_t nameLen)
{
  if (nameLen >= PDB_TABLE_NAME_LEN)
    return false;

  return ValidName(pTabName, nameLen);
}

bool StringTool::ValidColumnName(const char* pColName, size_t nameLen)
{
  if (nameLen >= PDB_FILED_NAME_LEN)
    return false;

  return ValidName(pColName, nameLen);
}
...
bool StringTool::ComparyNoCase(const char* pStr1, const char* pStr2)
{
  ...
  do {
    ...
    c1 = (unsigned char)upper_to_lower_char[*s1++];
    c2 = (unsigned char)upper_to_lower_char[*s2++];
...
bool StringTool::ComparyNoCase(const char* pStr1, size_t len1, const char* pStr2, size_t len2)
{
  if (len1 != len2)
    return false;
...
bool StringTool::ComparyNoCase(const std::string& str1, const char* pStr2, size_t len2)
{
  if (str1.size() != len2)
    return false;

  return ComparyNoCase(str1.c_str(), str1.size(), pStr2, len2);
}
...
bool StringTool::StartWith(const char* pStr, const char* pStartPart)
{
...
bool StringTool::StartWithNoCase(const char* pStr, const char* pStartPart)
{
  if (pStr == nullptr || pStartPart == nullptr)
    return false;
...
bool StringTool::ValidName(const char* pName, size_t nameLen)
{
  ...
  while (nameLen--)
  {
    if (allow_name_char[(uint8_t)(*pName++)] == 0)
      return false;
  }
...
uint64_t StringTool::CRC64(const void* pData, size_t len)
{
  return CRC64(pData, len, 0, 0);
}

uint64_t StringTool::CRC64(const void* pData, size_t len, size_t offset)
{
  return CRC64(pData, len, offset, 0);
}

uint64_t StringTool::CRC64(const void* pData, size_t len, size_t offset, uint64_t crc)
{
  const uint8_t* pTmp = (const uint8_t*)pData + offset;
...
uint32_t StringTool::CRC32(const char* pStr)
{
  return CRC64_TO_CRC32(CRC64(pStr));
}

uint32_t StringTool::CRC32(const void* pData, size_t len)
{
  return CRC64_TO_CRC32(CRC64(pData, len));
}

uint32_t StringTool::CRC32(const void* pData, size_t len, size_t offset)
{
  return CRC64_TO_CRC32(CRC64(pData, len, offset));
}
